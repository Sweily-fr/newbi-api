...
vi.mock('../../src/resolvers/clientAutomation.js', () => ({
  automationService: { executeAutomations: vi.fn().mockResolvedValue(undefined) },
}));
...
const mockContext = {
  user: { id: 'user-1', _id: 'user-1', name: 'Test User', email: 'test@test.com' },
  workspaceId,
  userRole: 'owner',
};
...
describe('Invoice Resolver - Query.invoice', () => {
  ...
  it('should return an invoice by id', async () => {
    ...
    const result = await resolver(null, { id: 'inv-1', workspaceId }, mockContext);
...
describe('Invoice Resolver - Query.invoices', () => {
  ...
  it('should apply status filter', async () => {
    ...
    await resolver(null, { workspaceId, status: 'PENDING', page: 1, limit: 10 }, mockContext);
  ...
  it('should apply search filter', async () => {
    ...
    await resolver(null, { workspaceId, search: 'Alpha', page: 1, limit: 10 }, mockContext);
...
describe('Invoice Resolver - Mutation.changeInvoiceStatus', () => {
  ...
  it('should throw on invalid transition COMPLETED to PENDING', async () => {
    ...
    await expect(
      resolver(null, { id: 'inv-1', workspaceId, status: 'PENDING' }, mockContext)
    ).rejects.toThrow();
  });
  ...
  it('should throw NOT_FOUND when invoice does not exist', async () => {
    ...
    await expect(
      resolver(null, { id: 'nonexistent', workspaceId, status: 'COMPLETED' }, mockContext)
    ).rejects.toThrow();
  });
});
