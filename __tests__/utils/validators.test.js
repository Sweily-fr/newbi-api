...
describe("REQUIRED_FIELDS_BY_COMPANY_STATUS", () => {
  ...
  it("should only require siret for AUTO_ENTREPRENEUR", () => {
    expect(isFieldRequiredForCompanyStatus("siret", "AUTO_ENTREPRENEUR")).toBe(
      true,
    );
    expect(
      isFieldRequiredForCompanyStatus("vatNumber", "AUTO_ENTREPRENEUR"),
    ).toBe(false);
    expect(isFieldRequiredForCompanyStatus("rcs", "AUTO_ENTREPRENEUR")).toBe(
      false,
    );
  });
