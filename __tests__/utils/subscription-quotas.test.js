...
describe("PLAN_QUOTAS configuration", () => {
  it("should define FREE plan with 5 monthly quota", () => {
    ...
    expect(PLAN_QUOTAS.FREE.extraImportPrice).toBe(0.3);
  ...
  it("should define TPE plan", () => {
    expect(PLAN_QUOTAS.TPE).toBeDefined();
    expect(PLAN_QUOTAS.TPE.monthlyQuota).toBeGreaterThan(0);
    expect(PLAN_QUOTAS.TPE.extraImportPrice).toBe(0.2);
  });

  it("should define ENTREPRISE plan", () => {
    expect(PLAN_QUOTAS.ENTREPRISE).toBeDefined();
    expect(PLAN_QUOTAS.ENTREPRISE.monthlyQuota).toBeGreaterThan(0);
  ...
  it("should define UNLIMITED plan with very high quota", () => {
    ...
    expect(PLAN_QUOTAS.UNLIMITED.extraImportPrice).toBe(0.1);
  });
  ...
  it("should have increasing or equal monthly quotas as plan tier increases", () => {
    const quotas = [
    ...
    for (let i = 1; i < quotas.length; i++) {
      expect(quotas[i]).toBeGreaterThanOrEqual(quotas[i - 1]);
    }
  });
});
