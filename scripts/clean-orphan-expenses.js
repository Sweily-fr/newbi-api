...
async function cleanOrphanExpenses() {
  let client;
  ...
  try {
    ...
    if (orphanExpenses > 0) {
      ...
      const orphanTotalAmount = orphanAmountResult.length > 0 ? orphanAmountResult[0].totalAmount : 0;
      console.log(`💰 Montant total des dépenses orphelines: ${orphanTotalAmount}€`);

      // 3. Supprimer les dépenses orphelines
      ...
      console.log(`💰 Montant récupéré: ${orphanTotalAmount}€`);
