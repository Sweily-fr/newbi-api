...
try {
  ...
  console.log(
    "⚠️  Impossible de charger ecosystem.config.cjs, utilise MONGODB_URI depuis .env"
  );
...
async function activateTrialForAllOrganizations(dryRun = true) {
  try {
    ...
    organizations.forEach((org, index) => {
      ...
      console.log(
        `   Avant : hasUsedTrial=${org.hasUsedTrial}, isTrialActive=${org.isTrialActive}`
      );
    });
    ...
    if (dryRun) {
      ...
      console.log(
        "Pour exécuter réellement, lance : node scripts/activate-trial-all-organizations.js --confirm\n"
      );
      return;
    }
    ...
    updatedOrgs.forEach((org, index) => {
      ...
      if (isValid) {
        ...
        errorCount++;
        console.log(
          `❌ ${index + 1}. ${org.name || "Sans nom"} - Mise à jour incomplète`
        );
      }
    });
