...
async function handleStripeWebhook(event) {
  try {
    logger.debug("Événement Stripe reçu:", event.type);
    logger.debug(
...
async function handleSubscriptionCreated(subscription) {
  try {
    logger.debug(
      "Traitement de subscription.created pour le client:",
    ...
    if (!user) {
      console.error(
        `Utilisateur non trouvé pour le customer ID: ${subscription.customer}`,
      );
      return { status: "error", message: "Utilisateur non trouvé" };
    }

    logger.debug(`Utilisateur trouvé: ${user.email}`);
    ...
    logger.debug(`Abonnement mis à jour pour l'utilisateur ${user.email}`);
...
async function handleSubscriptionUpdated(subscription) {
  try {
    logger.debug(
      "Traitement de subscription.updated pour le client:",
      subscription.customer,
    );
    ...
    logger.debug(
      `Utilisateur trouvé: ${user.email}, statut de l'abonnement: ${subscription.status}`,
    );
    ...
    // Mettre à jour les champs d'abonnement
    user.subscription.licence = ["active", "trialing"].includes(
      subscription.status,
    );
    user.subscription.trial = subscription.status === "trialing";
    ...
    logger.debug(
      `Abonnement mis à jour pour l'utilisateur ${user.email}, licence: ${user.subscription.licence}, trial: ${user.subscription.trial}`,
    );
...
async function handleSubscriptionDeleted(subscription) {
  try {
    logger.debug(
      "Traitement de subscription.deleted pour le client:",
      subscription.customer,
    );
    ...
    logger.debug(`Abonnement supprimé pour l'utilisateur ${user.email}`);
...
async function handlePendingUpdateApplied(subscription) {
  ...
  return {
    status: "success",
    message: "Mise à jour en attente appliquée avec succès",
  };
}
...
async function handleTrialWillEnd(subscription) {
  ...
  return {
    status: "success",
    message: "Notification de fin de période d'essai traitée",
  };
}
...
async function findUserByCustomerId(customerId) {
  logger.debug("Recherche d'utilisateur avec customerId:", customerId);
  ...
  try {
    ...
    if (user) {
      logger.debug("Utilisateur trouvé avec subscription.stripeCustomerId");
      return user;
    }
    ...
    if (user) {
      logger.debug("Utilisateur trouvé avec stripeId");
    ...
    try {
      logger.debug(
        "Tentative de récupération des informations client depuis Stripe",
      );
      const customer = await stripe.customers.retrieve(customerId);

      if (customer && customer.email) {
        logger.debug(`Email du client Stripe trouvé: ${customer.email}`);

        // Chercher l'utilisateur par email
        user = await User.findOne({ email: customer.email });

        if (user) {
          logger.debug(`Utilisateur trouvé par email: ${customer.email}`);
          ...
          logger.debug(`StripeCustomerId ajouté à l'utilisateur ${user.email}`);
          return user;
        } else {
          logger.debug(
            `Aucun utilisateur trouvé avec l'email: ${customer.email}`,
          );
        }
      } else {
        logger.debug(
          "Aucun email trouvé dans les informations du client Stripe",
        );
      }
    } catch (stripeError) {
      console.error(
        "Erreur lors de la récupération des informations client depuis Stripe:",
        stripeError.message,
      );
    }

    logger.debug("Aucun utilisateur trouvé avec ce customerId");
    return null;
  } catch (error) {
...
async function handleCheckoutSessionCompleted(session) {
  try {
    logger.debug("Traitement du paiement pour la session:", session.id);

    // Vérifier que le paiement est bien réussi
    if (session.payment_status !== "paid") {
      logger.debug("Paiement non effectué pour la session:", session.id);
    ...
    if (!fileTransferId) {
      ...
      return {
        status: "error",
        message: "ID de transfert de fichier manquant",
      };
    }
    ...
    if (!updatedTransfer) {
      console.error(
        "Transfert de fichier non trouvé avec l'ID:",
        fileTransferId,
      );
    ...
    logger.debug(
      "Paiement enregistré avec succès pour le transfert:",
      fileTransferId,
    );
    return {
      status: "success",
...
async function handleInvoicePaymentSucceeded(invoice) {
  try {
    logger.debug(
      "Traitement de invoice.payment_succeeded pour le client:",
      invoice.customer,
    );

    // Vérifier que c'est bien un paiement d'abonnement
    if (!invoice.subscription) {
      logger.debug("Pas un paiement d'abonnement, ignoré");
    ...
    if (paymentAmount <= 0) {
      logger.debug("Montant nul ou négatif, ignoré");
    ...
    if (!user) {
      logger.debug(
        "Utilisateur non trouvé pour le customer:",
        invoice.customer,
      );
      return { status: "error", message: "Utilisateur non trouvé" };
    }
    ...
    if (!user.referredBy) {
      logger.debug(
        "Utilisateur non référé par un partenaire, pas de commission",
      );
      return {
        status: "ignored",
        message: "Utilisateur non référé par un partenaire",
      };
    }

    logger.debug(`Utilisateur référé par le code: ${user.referredBy}`);
    ...
    if (!partner) {
      logger.debug("Partenaire non trouvé pour le code:", user.referredBy);
      return { status: "error", message: "Partenaire non trouvé" };
    }

    logger.debug(`Partenaire trouvé: ${partner.email}`);
    ...
    logger.debug(
      `Palier: ${tier.name}, Taux: ${commissionRate}%, Commission: ${commissionAmount}€`,
    );
    ...
    logger.debug(
      `Commission créée: ${commission._id} - ${commissionAmount}€ pour ${partner.email}`,
    );
