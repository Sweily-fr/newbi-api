...
/**
 * Check if a share (instance or plain object) has password protection.
 * Single source of truth for both virtual and field resolver.
 */
export function hasPasswordProtection(shareLike) {
  return Boolean(shareLike?.passwordHash || shareLike?.password);
}
...
const publicBoardShareSchema = new mongoose.Schema(
  {
    ...
    password: {
      type: String,
      default: null, // @deprecated — plaintext, kept for migration. Remove in phase 2.
    },
    passwordHash: {
      type: String,
      default: null,
...
publicBoardShareSchema.virtual("hasPasswordSet").get(function () {
  return hasPasswordProtection(this);
});

const BCRYPT_COST = 12;

publicBoardShareSchema.methods.setPassword = async function (plainPassword) {
  if (!plainPassword) {
    this.passwordHash = null;
    this.password = null;
    this.passwordHashedAt = null;
    return;
  }
  this.passwordHash = await bcrypt.hash(plainPassword, BCRYPT_COST);
  this.passwordHashedAt = new Date();
  this.password = null;
};

publicBoardShareSchema.methods.verifyPassword = async function (plainPassword) {
  ...
  if (this.password) {
    const provided = Buffer.from(plainPassword);
    const stored = Buffer.from(this.password);
...
publicBoardShareSchema.methods.recordVisit = async function (email) {
  const existingVisitor = this.visitors.find(
...
// Création du modèle
const PublicBoardShare = mongoose.model(
  "PublicBoardShare",
  publicBoardShareSchema,
);
