...
const importedInvoiceSchema = new mongoose.Schema(
  {
    ...
    status: {
      type: String,
      enum: [
        "UPLOADED", // PDF uploadé sans OCR (quota épuisé ou erreur OCR) — saisie manuelle attendue
        "PENDING_REVIEW", // OCR effectué, en attente de vérification
...
importedInvoiceSchema.index({
  workspaceId: 1,
  totalTTC: 1,
  "vendor.normalizedName": 1,
});
...
importedInvoiceSchema.statics.findPotentialDuplicates = async function (
  workspaceId,
  invoiceNumber,
  vendorName,
  totalTTC,
...
const ImportedInvoice = mongoose.model(
  "ImportedInvoice",
  importedInvoiceSchema,
);
