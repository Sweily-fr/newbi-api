...
const companyInfoSchema = new mongoose.Schema({
  ...
  phone: {
    ...
    match: [
      PHONE_FR_REGEX,
      "Veuillez fournir un numéro de téléphone français valide (ex: 06 12 34 56 78)",
    ],
  },
  email: {
    type: String,
  ...
  siren: {
    type: String,
    trim: true,
    match: [/^\d{9}$/, "Veuillez fournir un numéro SIREN valide (9 chiffres)"],
  },
  siret: {
    type: String,
    trim: true,
    match: [
      SIRET_REGEX,
      "Veuillez fournir un numéro SIRET valide (14 chiffres)",
    ],
    validate: {
      validator: function (v) {
        // Validation conditionnelle basée sur le statut juridique
        const requiredForStatuses = [
          "SARL",
          "SAS",
          "EURL",
          "SASU",
  ...
  vatNumber: {
    ...
    validate: {
      ...
      message:
        "Le numéro de TVA doit être au format FR (ex: FR12345678901) ou vide",
    },
  },
  bankDetails: {
  ...
  capitalSocial: {
    ...
    match: [
      CAPITAL_SOCIAL_REGEX,
      "Veuillez fournir un capital social valide (ex: 10000)",
    ],
    validate: {
      validator: function (v) {
        // Validation conditionnelle basée sur le statut juridique
  ...
  rcs: {
    ...
    match: [
      RCS_REGEX,
      "Veuillez fournir un RCS valide (ex: Paris B 123 456 789)",
    ],
    validate: {
      validator: function (v) {
        // Validation conditionnelle basée sur le statut juridique
