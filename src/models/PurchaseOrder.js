...
const { PURCHASE_ORDER_STATUS, DISCOUNT_TYPE } = require('./constants/enums');
...
purchaseOrderSchema.index({ 'client.name': 1 });
purchaseOrderSchema.index({ status: 1 });
purchaseOrderSchema.index({ issueDate: -1 });

// Index unique pour garantir l'unicité des numéros de bon de commande par utilisateur
purchaseOrderSchema.index({ number: 1, createdBy: 1 }, { unique: true });
...
// Création d'un index composé pour garantir l'unicité des numéros de bon de commande par année
...
purchaseOrderSchema.statics.numberExistsForYear = async function(number, userId, year) {
