...
const importedPurchaseOrderSchema = new mongoose.Schema(
  {
    ...
    status: {
      type: String,
      enum: ["PENDING_REVIEW", "VALIDATED", "REJECTED", "ARCHIVED"],
      default: "VALIDATED",
      index: true,
    },
    ...
    // Lien vers le Client CRM correspondant (créé/matché à l'import)
    clientId: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "Client",
      default: null,
...
importedPurchaseOrderSchema.index({
  workspaceId: 1,
...
importedPurchaseOrderSchema.statics.findPotentialDuplicates = async function (
  workspaceId,
  poNumber,
  vendorName,
  totalTTC,
...
const ImportedPurchaseOrder = mongoose.model(
  "ImportedPurchaseOrder",
  importedPurchaseOrderSchema,
);
