...
userInvitedSchema.pre("save", async function (next) {
  ...
  if (this.isModified("boardsAccess")) {
    this.stats.totalBoardsAccessed = this.boardsAccess.filter(
      (b) => b.status === "active",
    ).length;
  }
...
userInvitedSchema.methods.addBoardAccess = async function (
  boardId,
  shareId,
  workspaceId,
) {
...
userInvitedSchema.methods.banFromBoard = async function (
  boardId,
  reason = null,
) {
...
userInvitedSchema.methods.validateSessionToken = function (token) {
  ...
  if (!timingSafeStringEqual(this.sessionToken, token)) {
    return false;
  }
