...
const apiMetricSchema = new mongoose.Schema(
  ...
  },
  {
    ...
    // Le champ métier "errors" est un nom réservé Mongoose ; assumé, on
    // coupe le warning au boot.
...
// Méthodes d'instance
apiMetricSchema.methods.addRequest = function (
  responseTime,
  success = true,
  cost = 0,
) {
...
// Méthodes statiques
apiMetricSchema.statics.findOrCreate = async function (
  provider,
  endpoint,
  method,
  workspaceId,
...
apiMetricSchema.statics.getProviderStats = function (
  provider,
  startDate,
  endDate,
) {
