 ...
 */
export async function createDefaultClientLists(
  ClientList,
  workspaceId,
  userId,
) {
  try {
    ...
    if (existingLists > 0) {
      logger.debug(
        `Les listes par défaut existent déjà pour le workspace ${workspaceId}`,
      );
      return;
    }
    ...
    logger.debug(
      `✅ ${listsToCreate.length} listes par défaut créées pour le workspace ${workspaceId}`,
    );
  } catch (error) {
