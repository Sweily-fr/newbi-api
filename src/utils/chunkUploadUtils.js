  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      writeStream.on("error", (err) => {
        console.error(
          `Erreur lors de l'écriture du chunk ${chunkIndex} pour le fichier ${fileId}:`,
          err,
        );
        reject(err);
      });
      ...
      stream.on("error", (err) => {
        console.error(
          `Erreur lors de la lecture du chunk ${chunkIndex} pour le fichier ${fileId}:`,
          err,
        );
        reject(err);
      });
...
export const reconstructFile = async (
  ...
  try {
    ...
    logger.debug(
      `Fichier reconstruit - fileId: ${fileId}, fileName: ${fileName}, uniqueFilename: ${uniqueFilename}`,
    );
