...
function mapLegalFormToCompanyStatus(legalForm) {
  ...
  const sortedStatuses = [...VALID_COMPANY_STATUSES].sort(
...
function mapActivityToTransactionCategory(activityCategory) {
  ...
  if (VALID_TRANSACTION_CATEGORIES.includes(activityCategory))
...
export function mapOrganizationToCompanyInfo(organization) {
  if (!organization) {
    throw new Error(
      "Organization requise pour mapper les informations d'entreprise",
    );
  }
  ...
  const companyInfo = {
    ...
    website:
      organization.website && URL_REGEX.test(organization.website)
