...
const requireCompanyInfo = (resolver) => {
  return async (parent, args, context, info) => {
    ...
    if (!user) {
      logger.warn(
        "Tentative d'accès sans authentification aux fonctionnalités d'entreprise",
      );
      throw new AppError(
    ...
    // Récupérer le workspaceId depuis les arguments, le contexte ou le header organizationId
    let finalWorkspaceId =
      args.workspaceId || workspaceId || context.organizationId;
    ...
    if (!finalWorkspaceId && args.id) {
      try {
        ...
        logger.error(
          "Erreur lors de la récupération du workspaceId depuis le document:",
          error,
        );
      }
    }
    ...
    if (!finalWorkspaceId) {
      throw new AppError("workspaceId requis", ERROR_CODES.BAD_REQUEST, 400);
    }
    ...
    try {
      ...
      organization = await organizationCollection.findOne({
        _id: new mongoose.Types.ObjectId(finalWorkspaceId),
      });
    } catch (error) {
    ...
    if (!organization) {
      logger.warn(
        `Organisation non trouvée pour workspaceId: ${finalWorkspaceId}`,
      );
      throw new AppError(
    ...
    if (!isCompanyInfoComplete(organization)) {
      logger.warn(
        `WorkspaceId ${finalWorkspaceId} - Informations d'entreprise incomplètes`,
        {
...
const validateCompanyInfo = async (workspaceId) => {
  ...
  if (!organization) {
    throw new AppError("Organisation non trouvée", ERROR_CODES.NOT_FOUND, 404);
  }
