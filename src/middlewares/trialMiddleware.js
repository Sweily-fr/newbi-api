  ...
  return async (parent, args, context, info) => {
    ...
    if (context.user && context.user.id) {
      try {
        ...
        await TrialService.checkAndUpdateTrialStatus(context.user.id);
      } catch (error) {
        // Logger l'erreur mais ne pas bloquer la requête
        logger.warn(`Erreur lors de la vérification de la période d'essai pour l'utilisateur ${context.user.id}:`, error);
      }
    }
...
export const requirePremiumAccess = (resolver) => {
  return async (parent, args, context, info) => {
    ...
    try {
      ...
      logger.error(`Erreur lors de la vérification de l'accès premium pour l'utilisateur ${context.user.id}:`, error);
...
export const withTrialLimits = (resolver) => {
  return async (parent, args, context, info) => {
    ...
    try {
      ...
      logger.error(`Erreur lors de la vérification des limites d'essai pour l'utilisateur ${context.user.id}:`, error);
