...
export async function syncConnection(connectionId) {
  ...
  try {
    ...
    logger.info(
      `[CalendarSync] Démarrage sync pour connexion ${connectionId} (${connection.provider})`,
    );
    const externalEvents = await provider.fetchEvents(connection);
    logger.info(
      `[CalendarSync] ${externalEvents.length} événement(s) récupéré(s) depuis ${connection.provider}`,
    );
    ...
    for (const pe of pushedEvents) {
      for (const link of pe.externalCalendarLinks) {
        if (
          link.calendarConnectionId.toString() === connection._id.toString()
        ) {
    ...
    for (const eventData of externalEvents) {
      try {
        ...
        errors++;
        logger.warn(
          `[CalendarSync] Erreur upsert événement "${eventData.title}":`,
          eventError.message,
        );
      }
    }
    ...
    for (const pushedEvent of pushedEvents) {
      ...
      if (staleLinks.length > 0) {
        ...
        logger.info(
          `[CalendarSync] Removed ${staleLinks.length} stale link(s) from event "${pushedEvent.title}" (${pushedEvent._id})`,
        );
      }
    }
    ...
    if (unlinked > 0) {
      logger.info(
        `[CalendarSync] Cleaned up ${unlinked} stale externalCalendarLink(s) for connection ${connectionId}`,
      );
    }
    ...
    logger.info(
      `[CalendarSync] Sync terminée pour ${connectionId}: +${created} ~${updated} -${deleted} ⊘${unlinked} ⤬${skipped} (${errors} erreur(s))`,
    );
    ...
    logger.error(
      `Calendar sync failed for connection ${connectionId}:`,
      error.message,
    );
...
export async function syncAllActiveConnections({
  ...
  logger.info(
    `Starting calendar sync for ${connections.length} active connections${excludeWebhookActive ? " (excluding webhook-active)" : ""}`,
  );
  ...
  for (const connection of connections) {
    try {
      ...
      failCount++;
      logger.warn(
        `Sync failed for connection ${connection._id} (${connection.provider}): ${error.message}`,
      );
    }
  }

  logger.info(
    `Calendar sync completed: ${successCount} succeeded, ${failCount} failed`,
  );
  return { successCount, failCount, total: connections.length };
}

/**
 * Push a Newbi event to all the user's autoSync-enabled active calendar
 * connections (fire-and-forget). Used when an event is created automatically
 * by the system (e.g. invoice due dates), not via the manual createEvent
 * mutation that already handles its own auto-push.
 */
export async function autoPushEventToConnections(eventId, userId) {
  ...
  try {
    ...
    for (const conn of connections) {
      try {
        await pushEventToCalendar(eventId, conn._id);
        logger.info(
          `[autoPush] Pushed event ${eventId} to ${conn.provider} (${conn._id})`,
        );
      } catch (err) {
        logger.error(
          `[autoPush] Failed pushing event ${eventId} to ${conn.provider} (${conn._id}): ${err.message}`,
        );
      }
    }
  } catch (err) {
    logger.error(
      `[autoPush] Failed loading autoSync connections for user ${userId}: ${err.message}`,
    );
  }
}
...
export async function pushEventToCalendar(eventId, connectionId) {
  ...
  if (!connection || connection.status === "disconnected")
    throw new Error("Connexion calendrier non trouvée");
  ...
  logger.info(
    `Event ${eventId} pushed to ${connection.provider} calendar (${externalEventId})`,
  );
  return event;
}
...
export async function deleteEventFromExternalCalendars(event) {
  ...
  for (const link of links) {
    try {
      const connection = await CalendarConnection.findById(
        link.calendarConnectionId,
      );
      if (!connection || connection.status === "disconnected") {
        logger.warn(
          `[CalendarSync] Skipping delete propagation — connection ${link.calendarConnectionId} not found or disconnected`,
        );
        continue;
      }
      ...
      logger.info(
        `[CalendarSync] Propagated delete to ${connection.provider} (${link.externalEventId})`,
      );
    } catch (error) {
      logger.error(
        `[CalendarSync] Failed to propagate delete to ${link.provider} (${link.externalEventId}):`,
        error.message,
      );
    }
  }
...
export async function updateEventInExternalCalendars(event) {
  ...
  for (const link of links) {
    try {
      ...
      if (!connection || connection.status === "disconnected") {
        logger.warn(
          `[CalendarSync] Skipping update propagation — connection ${link.calendarConnectionId} not found or disconnected`,
        );
        continue;
      }
      ...
      logger.info(
        `[CalendarSync] Propagated update to ${connection.provider} (${link.externalEventId})`,
      );
    } catch (error) {
      logger.error(
        `[CalendarSync] Failed to propagate update to ${link.provider} (${link.externalEventId}):`,
        error.message,
      );
    }
  }
...
export async function disconnectCalendar(connectionId) {
  ...
  logger.info(
    `Deleted ${deleteResult.deletedCount} external events for connection ${connectionId}`,
  );
