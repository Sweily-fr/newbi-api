...
class ClaudeVisionOcrService {
  ...
  async batchProcessDocuments(documents, cacheService = null) {
    ...
    const startTime = Date.now();
    logger.debug(`🚀 Traitement batch de ${documents.length} factures...`);
      ...
      if (cacheService) {
        try {
          ...
          if (cached && cached.extractedData) {
            logger.debug(`💾 Cache HIT: ${doc.fileName}`);
            cachedResults.push({
              index: doc.index,
              url: doc.url,
              fileName: doc.fileName,
    ...
    logger.debug(`   - Depuis cache: ${cachedResults.length}`);
    logger.debug(
      `   - OCR traités: ${ocrResults.filter((r) => r.success).length}`,
    );
    logger.debug(`   - Échecs: ${allResults.filter((r) => !r.success).length}`);
