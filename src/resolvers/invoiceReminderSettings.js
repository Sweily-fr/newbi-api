...
{companyName}`,
  ...
  Mutation: {
     ...
     */
    updateInvoiceReminderSettings: requireWrite('invoices')(async (_, { input }, context) => {
      const { user, workspaceId, organization } = context;
      ...
      if (input.firstReminderDays && input.firstReminderDays < 1) {
        throw new UserInputError('Le délai de première relance doit être au moins 1 jour');
      }

      if (input.secondReminderDays && input.secondReminderDays < 1) {
        throw new UserInputError('Le délai de deuxième relance doit être au moins 1 jour');
      }

      if (input.firstReminderDays && input.secondReminderDays && 
          input.secondReminderDays <= input.firstReminderDays) {
        throw new UserInputError('Le délai de deuxième relance doit être supérieur au délai de première relance');
      }

      if (input.useCustomSender && !input.customSenderEmail) {
        throw new UserInputError('Email personnalisé requis si useCustomSender est activé');
      }
