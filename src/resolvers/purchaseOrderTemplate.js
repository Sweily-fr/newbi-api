  ...
  Query: {
    purchaseOrderTemplates: withWorkspace(
      async (_, { workspaceId }, { workspaceId: contextWorkspaceId }) => {
        ...
        return PurchaseOrderTemplate.find({ workspaceId: finalWorkspaceId }).sort({ createdAt: -1 });
      }
    ),
  },
  ...
  Mutation: {
    savePurchaseOrderAsTemplate: withWorkspace(
      async (_, { input, workspaceId }, { user, workspaceId: contextWorkspaceId }) => {
        ...
        const purchaseOrder = await PurchaseOrder.findOne({ _id: purchaseOrderId, workspaceId: finalWorkspaceId });
        ...
        const template = new PurchaseOrderTemplate({
          ...
          termsAndConditionsLink: purchaseOrder.termsAndConditionsLink || undefined,
          termsAndConditionsLinkTitle: purchaseOrder.termsAndConditionsLinkTitle || undefined,
          ...
          shipping: purchaseOrder.shipping ? {
            ...
            shippingAddress: purchaseOrder.shipping.shippingAddress || undefined,
            shippingAmountHT: purchaseOrder.shipping.shippingAmountHT || 0,
            shippingVatRate: purchaseOrder.shipping.shippingVatRate != null ? purchaseOrder.shipping.shippingVatRate : 20,
          } : undefined,
        ...
        logger.info(`[PurchaseOrderTemplate] Template "${name}" created from purchase order ${purchaseOrderId}`);
        return saved;
      }
    ),
    ...
    deletePurchaseOrderTemplate: withWorkspace(
      async (_, { id, workspaceId }, { workspaceId: contextWorkspaceId }) => {
        const finalWorkspaceId = workspaceId || contextWorkspaceId;
        const result = await PurchaseOrderTemplate.findOneAndDelete({ _id: id, workspaceId: finalWorkspaceId });
