    ...
    uploadFileChunk: isAuthenticated(
      async (
        ...
        try {
          ...
          const chunkInfo = await saveChunk(
            chunk,
            fileId,
            chunkIndex,
            fileName
          ...
          if (allChunksReceived) {
            ...
            fileInfo = await reconstructFile(
              fileId,
              fileName,
              totalChunks,
              user.id
      ...
      async (_, { fileIds, input }, { user }) => {
        try {
          ...
          const isPaymentRequired =
            input?.isPaymentRequired || input?.requirePayment || false;
          ...
          const transferHash = `${user.id}-${totalSize}-${filesInfo
