...
const kanbanResolvers = {
  ...
  Mutation: {
    ...
    deleteKanbanBoard: isAuthenticated(async (_, { id }, { user }) => {
      const result = await KanbanBoard.deleteOne({
        _id: id,
        createdBy: user.id,
      });
    ...
    addKanbanColumn: isAuthenticated(
      async (_, { boardId, input }, { user }) => {
        const board = await KanbanBoard.findOne({
          _id: boardId,
          createdBy: user.id,
        });
    ...
    updateKanbanColumn: isAuthenticated(
      async (_, { boardId, columnId, input }, { user }) => {
        const board = await KanbanBoard.findOne({
          _id: boardId,
          $or: [{ createdBy: user.id }, { members: user.id }],
        });
    ...
    deleteKanbanColumn: isAuthenticated(
      async (_, { boardId, columnId }, { user }) => {
        const board = await KanbanBoard.findOne({
          _id: boardId,
          createdBy: user.id,
        });
    ...
    reorderKanbanColumns: isAuthenticated(
      async (_, { boardId, columnIds }, { user }) => {
        const board = await KanbanBoard.findOne({
          _id: boardId,
          createdBy: user.id,
        });
    ...
    updateKanbanTask: isAuthenticated(
      async (_, { boardId, taskId, input }, { user }) => {
        const board = await KanbanBoard.findOne({
          _id: boardId,
          $or: [{ createdBy: user.id }, { members: user.id }],
        });
    ...
    deleteKanbanTask: isAuthenticated(
      async (_, { boardId, taskId }, { user }) => {
        const board = await KanbanBoard.findOne({
          _id: boardId,
          $or: [{ createdBy: user.id }, { members: user.id }],
        });
    ...
    moveKanbanTask: isAuthenticated(
      async (
        _,
        { boardId, taskId, sourceColumnId, targetColumnId, order },
        { user }
      ) => {
        const board = await KanbanBoard.findOne({
          _id: boardId,
    ...
    addKanbanTaskComment: isAuthenticated(
      async (_, { boardId, taskId, input }, { user }) => {
        const board = await KanbanBoard.findOne({
          _id: boardId,
          $or: [{ createdBy: user.id }, { members: user.id }],
    ...
    deleteKanbanTaskComment: isAuthenticated(
      async (_, { boardId, taskId, commentId }, { user }) => {
        const board = await KanbanBoard.findOne({
          _id: boardId,
          $or: [{ createdBy: user.id }, { members: user.id }],
        });
        ...
        for (const column of board.columns) {
          ...
          if (task) {
            ...
            if (
              ...
              board.createdBy.toString() !== user.id
            ) {
              throw new AppError(
