  ...
  Query: {
    quoteTemplates: withWorkspace(
      async (_, { workspaceId }, { workspaceId: contextWorkspaceId }) => {
        ...
        return QuoteTemplate.find({ workspaceId: finalWorkspaceId }).sort({ createdAt: -1 });
      }
    ),
  },
  ...
  Mutation: {
    saveQuoteAsTemplate: withWorkspace(
      async (_, { input, workspaceId }, { user, workspaceId: contextWorkspaceId }) => {
        ...
        const quote = await Quote.findOne({ _id: quoteId, workspaceId: finalWorkspaceId });
        ...
        const template = new QuoteTemplate({
          ...
          termsAndConditionsLinkTitle: quote.termsAndConditionsLinkTitle || undefined,
          ...
          shipping: quote.shipping ? {
            ...
            shippingVatRate: quote.shipping.shippingVatRate != null ? quote.shipping.shippingVatRate : 20,
          } : undefined,
        ...
        logger.info(`[QuoteTemplate] Template "${name}" created from quote ${quoteId}`);
        return saved;
      }
    ),
    ...
    deleteQuoteTemplate: withWorkspace(
      async (_, { id, workspaceId }, { workspaceId: contextWorkspaceId }) => {
        const finalWorkspaceId = workspaceId || contextWorkspaceId;
        const result = await QuoteTemplate.findOneAndDelete({ _id: id, workspaceId: finalWorkspaceId });
