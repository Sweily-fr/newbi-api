...
export default {
  Query: {
    ...
    fileTransferById: isAuthenticated(async (_, { id }, { user }) => {
      try {
        ...
        console.error('Erreur lors de la récupération du transfert de fichiers:', error);
        throw new ApolloError(
  ...
  Mutation: {
    ...
    createFileTransfer: isAuthenticated(async (_, { files, input = {} }, { user }) => {
      try {
        ...
        if (totalSize > MAX_FILE_SIZE) {
          ...
          throw new UserInputError('La taille totale des fichiers dépasse la limite de 100 GB');
        }
        ...
        console.error('Erreur lors de la création du transfert de fichiers:', error);
        throw new ApolloError(
    ...
    deleteFileTransfer: isAuthenticated(async (_, { id }, { user }) => {
      try {
        ...
        console.error('Erreur lors de la suppression du transfert de fichiers:', error);
        throw new ApolloError(
