...
const emailSignatureResolvers = {
  Query: {
    ...
    emailSignatures: isAuthenticated(
      async (_, { search, page = 1, limit = 20 }, { user }) => {
        ...
        const [signatures, totalCount] = await Promise.all([
          EmailSignature.find(query).sort(sortOptions).skip(skip).limit(limit),
  ...
  Mutation: {
    createEmailSignature: isAuthenticated(async (_, { input }, { user }) => {
      ...
      if (input.jobTitle && !NAME_REGEX.test(input.jobTitle)) {
        validationErrors.jobTitle =
          "Le titre du poste contient des caractères non autorisés";
      }
      ...
      if (
        input.phone &&
        input.phone.trim() !== "" &&
        !PHONE_FR_REGEX.test(input.phone)
      ) {
      ...
      if (
        ...
        !PHONE_FR_REGEX.test(input.mobilePhone)
      ) {
      ...
      if (input.socialLinks) {
        if (
          ...
          !URL_REGEX.test(input.socialLinks.linkedin)
        ) {
          validationErrors["socialLinks.linkedin"] =
            "Format d'URL LinkedIn invalide";
        }

        if (
          input.socialLinks.twitter &&
          !URL_REGEX.test(input.socialLinks.twitter)
        ) {
          validationErrors["socialLinks.twitter"] =
            "Format d'URL Twitter invalide";
        }

        if (
          input.socialLinks.facebook &&
          !URL_REGEX.test(input.socialLinks.facebook)
        ) {
          validationErrors["socialLinks.facebook"] =
            "Format d'URL Facebook invalide";
        }

        if (
          input.socialLinks.instagram &&
          !URL_REGEX.test(input.socialLinks.instagram)
        ) {
          validationErrors["socialLinks.instagram"] =
            "Format d'URL Instagram invalide";
        }
      }
      ...
      if (Object.keys(validationErrors).length > 0) {
        throw createValidationError(
          "Certains champs contiennent des erreurs de validation",
          validationErrors
        );
      }
      ...
      const isDefault =
        input.isDefault !== undefined ? input.isDefault : signatureCount === 0;
    ...
    updateEmailSignature: isAuthenticated(
      async (_, { id, input }, { user }) => {
        ...
        if (input.profilePhotoBase64) {
          try {
            ...
            input.profilePhotoUrl = saveEmailSignaturePhoto(
              input.profilePhotoBase64
            );
          } catch (error) {
            console.error(
              "Erreur lors de l'upload de la photo de profil:",
