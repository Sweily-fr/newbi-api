  ...
  Query: {
    ...
    getEmailPreferences: isAuthenticated(async (_, __, { user }) => {
      try {
        ...
        logger.error(
          "Erreur lors de la récupération des préférences email:",
          error,
        );
        throw error;
      }
     ...
     */
    getEmailLogs: isAuthenticated(
      async (_, { workspaceId, status, limit = 30, offset = 0 }, { user }) => {
        try {
          ...
          logger.error(
            "Erreur lors de la récupération des logs d'emails:",
            error,
          );
          throw error;
        }
    ...
    testEmailReminder: isAuthenticated(async (_, { eventId }, { user }) => {
      try {
        ...
        const reminderType = event.emailReminder?.anticipation
          ? "anticipated"
          : "due";
    ...
    sendTestEmail: isAuthenticated(async (_, __, { user }) => {
      try {
        ...
        const testEvent = {
          ...
          description:
            "Ceci est un email de test pour vérifier que vos rappels fonctionnent correctement.",
          start: new Date(),
          end: new Date(),
          allDay: false,
        ...
        const { subject, html } = emailReminderService.generateEmailContent(
          testEvent,
          "due",
        );
...
// ✅ Phase A.3 — Subscription check sur toutes les mutations emailReminder
const originalEmailReminderMutations = emailReminderResolvers.Mutation;
emailReminderResolvers.Mutation = Object.fromEntries(
  Object.entries(originalEmailReminderMutations).map(([name, fn]) => [
    name,
