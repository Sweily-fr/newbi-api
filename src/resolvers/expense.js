...
const checkExpenseAccess = async (expenseId, userId) => {
  ...
  if (!expense) {
    throw new ForbiddenError('Vous n\'êtes pas autorisé à accéder à cette dépense');
  }
  return expense;
};
...
const saveUploadedFile = async (file, userId) => {
  ...
  const uploadDir = path.join(process.cwd(), 'public', 'uploads', 'expenses', userId.toString());
...
const expenseResolvers = {
  Query: {
    ...
    expenses: async (_, { startDate, endDate, category, status, search, tags, page = 1, limit = 10 }, { user }) => {
      if (!user) throw new ForbiddenError('Vous devez être connecté');
    ...
    expenseStats: async (_, { startDate, endDate }, { user }) => {
      ...
      const totalStats = stats[0].totalStats[0] || { totalAmount: 0, totalCount: 0 };
  ...
  Mutation: {
    ...
    createExpense: async (_, { input }, { user }) => {
      ...
      if (input.paymentDate) expenseData.paymentDate = new Date(input.paymentDate);
      ...
      try {
        ...
        if (input.processOCR) {
          try {
            ...
            if (!expense.ocrMetadata || Object.keys(expense.ocrMetadata).length === 0) {
              expense.ocrMetadata = {
        ...
        throw new ApolloError('Erreur lors de l\'ajout du fichier', 'FILE_UPLOAD_ERROR', { error });
      }
    },
    ...
    removeExpenseFile: async (_, { expenseId, fileId }, { user }) => {
      ...
      const fileIndex = expense.files.findIndex(file => file._id.toString() === fileId);
    ...
    updateExpenseOCRMetadata: async (_, { expenseId, metadata }, { user }) => {
      ...
      expense.ocrMetadata = {
        ...expense.ocrMetadata,
        ...metadata,
        invoiceDate: metadata.invoiceDate ? new Date(metadata.invoiceDate) : expense.ocrMetadata.invoiceDate
      };
    ...
    processExpenseFileOCR: async (_, { expenseId, fileId }, { user }) => {
      ...
      try {
        ...
        throw new ApolloError('Erreur lors du traitement OCR', 'OCR_PROCESSING_ERROR', { error });
      }
    },
    ...
    applyOCRDataToExpense: async (_, { expenseId }, { user }) => {
      ...
      if (!expense.ocrMetadata || Object.keys(expense.ocrMetadata).length === 0) {
        throw new UserInputError('Aucune donnée OCR disponible pour cette dépense');
      }
      ...
      if (expense.ocrMetadata.vatAmount) {
        ...
        if (expense.amount > 0) {
          expense.vatRate = (expense.vatAmount / (expense.amount - expense.vatAmount)) * 100;
        }
      }
