...
const integrationResolvers = {
    ...
    integrations: async (_, __, { user }) => {
      if (!user) {
        throw new AuthenticationError(
          "Vous devez être connecté pour accéder à vos intégrations",
        );
      }
      ...
      try {
        ...
        throw new Error(
          "Une erreur est survenue lors de la récupération des intégrations",
        );
      }
    },
  },
  ...
  Mutation: {
    ...
    connectStripe: async (_, { apiKey }, { user }) => {
      if (!user) {
        throw new AuthenticationError(
          "Vous devez être connecté pour connecter Stripe",
        );
      }
      ...
      if (!user.subscription || !user.subscription.licence) {
        throw new UserInputError(
          "Vous devez avoir une licence premium pour utiliser cette fonctionnalité",
        );
      }
      ...
      try {
        ...
        if (error.message === "Format de clé API Stripe invalide") {
          throw new UserInputError(
            "Format de clé API Stripe invalide. La clé doit commencer par sk_test_, sk_live_, pk_test_ ou pk_live_ suivi de caractères alphanumériques.",
          );
        }
        ...
        throw new Error(
          "Une erreur est survenue lors de la connexion à Stripe",
        );
      }
    },
    ...
    disconnectStripe: async (_, __, { user }) => {
      if (!user) {
        throw new AuthenticationError(
          "Vous devez être connecté pour déconnecter Stripe",
        );
      }
      ...
      try {
        ...
        throw new Error(
          "Une erreur est survenue lors de la déconnexion de Stripe",
        );
      }
    },
  },
...
// ✅ Phase A.4 — Subscription check on connectStripe mutation (exclude disconnectStripe)
const INTEGRATION_BLOCK = ["connectStripe"];
INTEGRATION_BLOCK.forEach((name) => {
  const original = integrationResolvers.Mutation[name];
  if (original) {
    integrationResolvers.Mutation[name] = async (
      parent,
      args,
      context,
      info,
