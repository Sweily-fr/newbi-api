  ...
  Mutation: {
    createInvoice: isAuthenticated(async (_, { input }, { user }) => {
      ...
      if (input.status !== "DRAFT") {
        ...
        if (
          lastDraftInvoice &&
          lastDraftInvoice.number &&
          lastDraftInvoice.number.startsWith("DRAFT-")
        ) {
          // Extraire le numéro du dernier brouillon
          const lastDraftNumber = parseInt(
            lastDraftInvoice.number.replace("DRAFT-", ""),
            10
          );
    ...
    updateInvoice: isAuthenticated(async (_, { id, input }, { user }) => {
      ...
      if (
        ...
        throw createResourceLockedError(
          "Facture",
          `une facture ${
            invoiceData.status === "COMPLETED" ? "terminée" : "annulée"
    ...
    changeInvoiceStatus: isAuthenticated(
      async (_, { id, status }, { user }) => {
        const invoice = await Invoice.findOne({
          _id: id,
          createdBy: user.id,
        }).populate("createdBy");
    ...
    markInvoiceAsPaid: isAuthenticated(
      async (_, { id, paymentDate }, { user }) => {
        const invoice = await Invoice.findOne({
          _id: id,
          createdBy: user.id,
        }).populate("createdBy");
